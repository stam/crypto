...
describe('The MockMarket', () => {
  ...
  describe('should update account totals', () => {
    ...
    it('for market orders when there are no future ticks', async () => {
      const START_FIAT = 7000;
      const market = new MockMarket({ accountValue: 0, accountFiat: START_FIAT });
    ...
    it('for buy market orders', async () => {
      ...
      const valueAfterFirstBuy = START_FIAT - ticks[0].last;
      expect(market.accountFiat).toBe(valueAfterFirstBuy);
      expect(market.accountValue).toBe(0);
